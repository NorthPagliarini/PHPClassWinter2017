...
-- create a separate table for customers if you want, just keep in mind
-- that you need to consider if you'll be tracking admins using the
-- same table and also keep it simple and don't try and have different
-- addresses for billing and shipping. 

-- create an orders table. Keep in mind that you need to track 
-- the data that applies to the entire order, so consider items
-- such as the customer id, the date the order was placed, shipping
-- and taxes paid, card data, ship date and tracking data.
-- Don't store the total for the order as that can be recalculated.

-- create an order items table. This table tracks the data related to the
-- particular item(s) being purchased. So, the product id and order id are
-- foreign keys, you want to track the price and quantity of the item
-- ordered. You also want to store the discounted amount. You do this because
-- prices change and if you have to give a refund in the future...
